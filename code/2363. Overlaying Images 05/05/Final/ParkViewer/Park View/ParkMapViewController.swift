...
class ParkMapViewController: UIViewController, MKMapViewDelegate {
  ...
  override func viewDidLoad() {
    ...
    mapView.addOverlay(ParkOverlay(park: park))
  ...
  func mapView(_ mapView: MKMapView, rendererFor overlay: MKOverlay) -> MKOverlayRenderer {
    ...
    guard let image = UIImage(named: "overlay_park") else {
      fatalError("Unable to create renderer - missing image")
    }
    let renderer = ParkMapOverlayRenderer(overlay: overlay, overlayImage: image)
    return renderer
