...
final class Rover: Codable, Equatable {
  ...
  init(from decoder: Decoder) throws {
    ...
    if let deleteBadSamples = decoder.userInfo[Rover.deleteBadSamplesKey] as? Bool,
      deleteBadSamples {
      rockSamples = try container.decode([FailableDecodeBox<RockSample>].self, forKey: .rockSamples)
        .compactMap { $0.model }
    } else {
