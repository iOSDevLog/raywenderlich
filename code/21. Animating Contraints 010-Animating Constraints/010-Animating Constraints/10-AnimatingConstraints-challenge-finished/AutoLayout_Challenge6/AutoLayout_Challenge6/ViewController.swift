...
class ViewController: UIViewController {
  ...
  @IBAction func imageTapped(sender: UITapGestureRecognizer) {
    ...
    var constraintToShrink: NSLayoutConstraint?
    var constraintToGrow: NSLayoutConstraint?
    if let selectedConstraint = selectedConstraint {
      constraintToShrink = selectedConstraint
    }
    if widthConstraint == selectedConstraint {
      ...
      constraintToGrow = widthConstraint
    ...
    // Make sure previous changes are laid out without animation
    view.layoutIfNeeded()

    // Make changes to constraints you want to animate (including adding or removing constraints)
    if let constraintToShrink = constraintToShrink {
      constraintToShrink.constant = 50
    }
    if let constraintToGrow = constraintToGrow {
      constraintToGrow.constant = 100
    }
