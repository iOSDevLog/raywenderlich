...
class CatDetailViewController: UIViewController {
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
    keyboardShowNotification = NotificationCenter.default.addObserver(forName: .UIKeyboardWillShow, object: nil, queue: OperationQueue.main) { [weak self] (notification) in
      self?.adjustViewForKeyboard(notification: notification)
    }
    keyboardHideNotification = NotificationCenter.default.addObserver(forName: .UIKeyboardWillHide, object: nil, queue: OperationQueue.main) { [weak self] (notification) in
      self?.adjustViewForKeyboard(notification: notification)
    }
  }
